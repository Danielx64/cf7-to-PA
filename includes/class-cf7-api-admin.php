...
class QS_CF7_api_admin{
  ...
  function add_integrations_tab($panels){
	...
	$integration_panel = array(
	  'title'    => __( 'Power Automate' , $this->textdomain ),
...
<?php
  ...
  public function qs_save_contact_form_details( $contact_form ){
	...
	$record_type = isset( $properties['wpcf7_api_data']['input_type'] ) ? $properties['wpcf7_api_data']['input_type'] : 'json';
	  $template =  $properties['json_template'];
  ...
  private function send_lead( $record , $debug = false , $method = 'GET' , $record_type = 'params'){
	...
	$url  = $record["url"];
	  ...
	  $args = array(
		...
		$args['headers']['setntfrom'] = home_url();
